...
export async function generateMetadata({ params }: Props) {
  ...
  const result = await getTermPageData("category", slug, 1, locale).catch(
    () => null
  )
  if (!result) return {}
  return createPageMetadata({
    locale,
    pathname: `/category/${result.term.slug}`,
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const result = await getTermPageData("category", slug, page, locale).catch(
    () => null
  )
  if (!result) notFound()
  return (
